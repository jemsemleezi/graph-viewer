    ...
    try:
        with sqlite3.connect(db_path, timeout=30) as conn:
            ...
            conn.execute("""
                ...
                SELECT id, CAST(from_id AS TEXT), CAST(to_id AS TEXT), type, instruction, condition, session_id, created_at
                FROM gm_edges_old
            """)
...
def get_stats():
    if _json_graph is not None:
        ...
        return jsonify({
            ...
            'communities': len(communities), 'types': type_dist
        })
    try:
        with get_db() as conn:
    ...
    try:
        if _json_graph is not None:
            ...
            conn.execute('''CREATE TABLE IF NOT EXISTS gm_nodes (
                id TEXT PRIMARY KEY, type TEXT, name TEXT, description TEXT, content TEXT,
                status TEXT DEFAULT 'active', community_id INTEGER, pagerank REAL,
                created_at INTEGER, updated_at INTEGER)''')
            conn.execute('''CREATE TABLE IF NOT EXISTS gm_edges (
                id TEXT PRIMARY KEY, from_id TEXT, to_id TEXT, type TEXT, instruction TEXT,
